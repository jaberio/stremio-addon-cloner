...
const STREMIO_API_BASE = process.env.STREMIO_API_URL || "https://api.strem.io/api";

const makeRequest = async (options: { path: string; params: object }) => {
    const res = await fetch(`${STREMIO_API_BASE}/${options.path}`, {
        method: "POST",
...
const getAuth = async (acc: Account) => {
    ...
    const account = await login(acc.email, acc.password);
